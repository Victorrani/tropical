...
for file in files:
    ...
    for i, season in enumerate(seasons):
        ...
        im = ax.contourf(atmospheric_balance_physics.longitude, atmospheric_balance_physics.latitude,
                          atmospheric_balance_physics,
                         transform=ccrs.PlateCarree(),
                         cmap='RdBu_r', levels=np.arange(-200, 201, 10), norm=norm, extend='both')
        ax.set_title(f'Atmospheric Balance - {seasons[i]}', loc='left')
        ax.coastlines()
    ...
    cbar.set_ticks(np.arange(-200, 201, 20))
    cbar.ax.tick_params(labelsize=10)
    fig.suptitle(f'Atmospheric Balance - {file.stem}', fontsize=16)

    fig.savefig(outdir /  f'atmospheric_balance_{file.stem}.png', dpi=300, bbox_inches='tight')
